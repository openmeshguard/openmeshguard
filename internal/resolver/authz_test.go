...
func TestResolverV2ResolveAuthz(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "CUSTOM DENY and ALLOW preserve the complete evaluation sequence",
			in: authzWorkload(ModeSidecar,
				authzPolicy("payments", "allow-api", "ALLOW", true),
				authzPolicy("payments", "deny-admin", "DENY", true),
				authzPolicy("payments", "external-check", "CUSTOM", true),
			...
			wantBroadAllow: &falseValue,
			wantIdentity:   &trueValue,
			wantPolicies:   []string{"payments/external-check", "payments/deny-admin", "payments/allow-api"},
			wantChain: []Step{
				authzDefaultStep(1),
				authzPolicyStepWant(2, "payments", "external-check", "evaluates CUSTOM authorization before additive DENY and ALLOW policies"),
				authzPolicyStepWant(3, "payments", "deny-admin", "adds DENY rules; any matching DENY overrides additive ALLOW policies"),
				authzPolicyStepWant(4, "payments", "allow-api", "adds explicit rules to the additive ALLOW union"),
			},
		},
		{

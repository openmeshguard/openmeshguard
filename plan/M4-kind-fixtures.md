...
- Exact corrected-branch `make kind-up e2e kind-down` from no Kind clusters, using `/tmp/openmeshguard review # state`: 46s, 27s, and 0s. A second fresh cluster then produced two consecutive `make e2e` runs in 24s and 35s with identical goldens and audit results; its setup and teardown took 43s and 0s.
...
- Final `make build test lint schema-test`: green; lint reported zero issues. The synthetic Kind/istioctl fallback test passes with the fix and fails against the previous helper. Missing-finding and missing-chain mutations are rejected, and generated Kind YAML round-trips special-character host paths. `go test -race ./deploy/rbac ./internal/output ./test/e2e`, shell syntax, actionlint, relative/special-character-state-path regression, and `git diff --check`: green.

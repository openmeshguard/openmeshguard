...
- Final `make build test lint schema-test`: green; lint reported zero issues. The synthetic Kind/istioctl fallback test passes with the fix and fails against the previous helper. `go test -race ./deploy/rbac ./internal/output`, shell syntax, actionlint, relative-state-path regression, and `git diff --check`: green.

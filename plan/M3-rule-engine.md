...
- Template selector validation tracks statically typed local variables, including direct and parenthesized assignments such as `{{$p := (.Posture)}}{{$p.Mtls.Effective}}`; a misspelled variable field is rejected during pack validation rather than aborting a later scan.
...
- Namespace-scope controls receive an explicit complete target set containing only known enrolled namespaces plus namespaces whose enrollment is unknown; workload context cannot backfill conclusively non-mesh namespaces into that set. A denied Pod list is treated as possible workload- and namespace-target loss as well as missing data-plane evidence, so both loaded scopes are named in that permission's `affectedControls`.

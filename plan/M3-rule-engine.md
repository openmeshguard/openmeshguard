...
- User remediation template paths are resolved canonically beneath the control-pack directory so symlinks cannot escape it. Message and remediation templates validate static selectors against the supported template shape during pack load, while dynamic `Params` and `Inventory` keys remain permitted.
- Namespace mesh enrollment preserves namespace-label evidence and aggregates observed workload enrollment without order-dependent last-write behavior. An unobserved workload cannot overwrite a conclusive sidecar or ambient namespace label with `unknown`.

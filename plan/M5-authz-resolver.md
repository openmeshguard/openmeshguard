...
- [x] Chain assertions in every table case. `Version()` becomes a composite (current tags: `mtls/v2,authz/v2`) — document the scheme in the resolver package doc.
...
- The resolver version is the stable ordered composite `mtls/v2,authz/v2`; only the subsystem tag whose semantics change is bumped. MG-AUTHZ-001..007 and MG-MTLS-007 ship as YAML/CEL data, and produced ports make MG-MTLS-002 evaluable.

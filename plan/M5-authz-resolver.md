...
- Two consecutive non-update `make e2e` runs both matched every `sidecar-basic` and `sidecar-authz` golden, passed the ClusterRole and namespace Role proofs, recorded exactly 220 approved scanner list events and no other calls, and completed in 48 seconds each.

...
- [ ] Match DestinationRule workload selectors and Sidecar visibility against client proxies.
- [ ] Project both singular `targetRef` and plural `targetRefs` attachments.
- [ ] Translate DestinationRule service-port overrides to destination workload ports.
- [ ] Collect EndpointSlices and preserve unknown versus observed-empty port evidence for selectorless Services.
- [ ] Apply DestinationRule lookup precedence: client namespace, service namespace, then root namespace.
- [ ] Distinguish identity-scoped ALLOW posture from broad or source-unscoped access for MG-AUTHZ-004.
- [ ] Preserve targetRef identity and bind each targetRef policy to its specific waypoint enforcement path.
- [ ] Return explicit unknown authorization posture for unknown or mixed data-plane membership.
- [ ] Treat `selector: {}` as matching every workload in policy scope.
- [ ] Treat ruleless DENY and CUSTOM policies as never matching.

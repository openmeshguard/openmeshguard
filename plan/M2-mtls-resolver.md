...
## Deliverables
- [ ] Precedence implementation: mesh-wide PA → namespace PA → workload-selector PA → portLevelMtls, UNSET inheritance handled correctly at each level.
- [ ] `mixed-by-port` computation and ByPort map population.
- [ ] DestinationRule interplay: client TLSMode DISABLE/SIMPLE against server STRICT ⇒ ClientTLSContradiction=true with both resources in the chain.
- [ ] Data-plane awareness: not-in-mesh workloads ⇒ MTLSNotInMesh; ambient handled to the extent inputs allow (ztunnel Tristate; full ambient controls in M6).
- [ ] Unknown propagation: MeshDefaults.Known=false or Unobserved inputs ⇒ MTLSUnknown with UnknownReason; never a guessed default.
...
- [ ] Chain assertions in every table case: order, kinds, and effects — not just the final enum.
- [ ] `Version()` returns `mtls/v1` semantics tag; bump rule documented in package doc.
